...
public class GloamHostConfigTests
{
    ...
    [Test]
    public void RuntimeConfig_ShouldBeSettable()
    {
        var config = new GloamHostConfig();
        var runtimeConfig = new RuntimeConfig(50, 16, false);
        config.RuntimeConfig = runtimeConfig;

        Assert.That(config.RuntimeConfig, Is.EqualTo(runtimeConfig));
    }
    ...
    public void InitializerSyntax_ShouldWork()
    {
        var config = new GloamHostConfig
        {
            AppName = "TestGame",
            AppVersion = "0.1.0",
            RuntimeConfig = new RuntimeConfig(100, 16),
        ...
        Assert.That(config.RuntimeConfig.SimulationStep, Is.EqualTo(100));
        Assert.That(config.LoaderType, Is.EqualTo(ContentLoaderType.FileSystem));
        Assert.That(config.LogLevel, Is.EqualTo(LogLevelType.Debug));
        Assert.That(config.EnableConsoleLogging, Is.False);
        Assert.That(config.EnableFileLogging, Is.True);
    ...
    [Test]
    public void RuntimeConfig_ShouldHaveDefaultInstance()
    {
        var config = new GloamHostConfig();
